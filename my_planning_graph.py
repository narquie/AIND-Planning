...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        # Look over all the actions available
        for i in self.all_actions:
            # Turn in to an action node
            actionNode = PgNode_a(i)
            # Check previous possible state nodes
            for j in actionNode.prenodes:
                # Compare against existing state nodes
                for k in self.s_levels[level]:
                    # Equal to each other?
                    if k.__eq__(j):
                        # Update parents, children, and add the action node
                        actionNode.parents.add(k)
                        k.children.add(actionNode)
                        self.a_levels[level].add(actionNode)
    ...
    def add_literal_level(self, level):
        ...
        # Check previous actions (to use function effnodes)
        for i in self.a_levels[level-1]:
            # Add all effnodes
            for j in i.effnodes:
                # Update parents, children, and add the state node
                i.children.add(j)
                j.parents.add(i)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        # Mutex if one node adds what another removes
        for i in node_a1.action.effect_add:
            for j in node_a2.action.effect_rem:
                if i == j:
                    return True
        for i in node_a2.action.effect_add:
            for j in node_a1.action.effect_rem:
                if i == j:
                    return True
        return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        """if node_a1.action.effect_add == node_a2.action.precond_neg or node_a1.action.effect_rem == node_a2.action.precond_pos:
            return True
        elif node_a2.action.effect_add == node_a1.action.precond_neg or node_a2.action.effect_rem == node_a1.action.precond_pos:
            return True"""

        # Mutex if one effect adds what another action doesn't want as precond
        for i in node_a1.action.effect_add:
            for j in node_a2.action.precond_neg:
                if i == j:
                    return True
        for i in node_a2.action.effect_add:
            for j in node_a1.action.precond_neg:
                if i == j:
                    return True
        # Mutex if one effect removes what another action does want as precond
        for i in node_a1.action.effect_rem:
            for j in node_a2.action.precond_pos:
                if i == j:
                    return True
        for i in node_a2.action.effect_rem:
            for j in node_a1.action.precond_pos:
                if i == j:
                    return True
        return False
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        # Mutex if state parents of actions are mutex
        for i in node_a1.parents:
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        :return: bool
        """
        # Mutex if nodes are the same symbol, but one is the absence of the other
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        :return: bool
        """
        # Mutex if all action parents are mutex of each other
    ...
    def h_levelsum(self) -> int:
        ...
        level_sum = 0
        break_2 = False
        # For all states of a goal
        for i in self.problem.goal:
            # Turn into PgNode
            node = PgNode_s(i, True)
            # Pull index for level_sum and list of states
            for index, j in enumerate(self.s_levels):
                # Pull individual state
                for k in j:
                    # If the state is equal to the goal, add the level to level_sum
                     if k.__eq__(node):
                         level_sum += index
                         break_2 = True
                         break
                # Breaks out of loop and goes on to next goal state if true
                if break_2:
                    break_2 = False
                    break
        return level_sum
